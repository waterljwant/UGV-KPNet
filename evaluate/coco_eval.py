...
def get_multiplier(img, inp_size=368.):
    ...
    return [x * inp_size / float(img.shape[0]) for x in scale_search]
...
def run_eval(image_dir, anno_dir, vis_dir, image_list_txt, model, preprocess):
    ...
    for i in range(len(img_ids)):
        ...
        to_plot, canvas, candidate, subset = decode_pose(oriImg, param, heatmap, paf)
        ...
        # cv2.waitKey(0)
        #
        # cv2.imshow('test-to_plot', to_plot)
        # cv2.waitKey(0)
