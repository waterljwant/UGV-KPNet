...
class Cocokeypoints(Dataset):
    def __init__(self, root, mask_dir, index_list, data, inp_size, feat_stride, preprocess='rtpose', transform=None,
        ...
        print("Cocokeypoints: init data loader")
    ...
    def get_ground_truth(self, meta, mask_miss):
        ...
        # crop_size_y = mask_miss.shape[1]  #
        # crop_size_x = mask_miss.shape[0]
        ...
        # print('mask_miss.shape-1', mask_miss.shape)  # (46, 46)

        # grid_y = mask_miss.shape[1]  #
        # grid_x = mask_miss.shape[0]
        ...
        # print('mask_miss.shape-2', mask_miss.shape)  # (46, 46, 1)

        heat_mask = np.repeat(mask_miss, 19, axis=2)
        # print('heat_mask.shape', heat_mask.shape)  # (46, 46, 19)
    ...
    def __getitem__(self, index):
        ...
        meta_data, img, mask_miss = aug_scale(meta_data, img, mask_miss, self.params_transform)  # TODO
        #
        meta_data, img, mask_miss = aug_rotate(meta_data, img, mask_miss, self.params_transform)
        #
        meta_data, img, mask_miss = aug_croppad(meta_data, img, mask_miss, self.params_transform)  # TODO
        #
        meta_data, img, mask_miss = aug_flip(meta_data, img, mask_miss, self.params_transform)

        meta_data = self.remove_illegal_joint(meta_data)

        heat_mask, heatmaps, paf_mask, pafs = self.get_ground_truth(meta_data, mask_miss)
