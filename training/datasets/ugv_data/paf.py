...
def putVecMaps(centerA, centerB, accumulate_vec_map, count, params_transform):
    ...
    min_x = max(int(round(min(centerA[0], centerB[0]) - thre_kik)), 0)
    max_x = min(int(round(max(centerA[0], centerB[0]) + thre_kik)), grid_x)
    min_y = max(int(round(min(centerA[1], centerB[1]) - thre_kik)), 0)
    max_y = min(int(round(max(centerA[1], centerB[1]) + thre_kik)), grid_y)
    ...
    # mask = limb_width < 0.9  # mask is 2D
