...
from network.ugv_post_refine import pallet_post_kp_list, pallet_post_3to4th
...
joint_to_limb_heatmap_relationship = [[0, 1], [1, 2], [2, 3], [3, 0]]

# PAF indices containing the x and y coordinates of the PAF for a given limb.
paf_xy_coords_per_limb = [[0, 1], [2, 3], [4, 5], [6, 7]]
...
colors = [[255, 0, 0], [85, 255, 0], [0, 170, 255], [0, 0, 255]]  # BGR

NUM_JOINTS = 4
NUM_LIMBS = len(joint_to_limb_heatmap_relationship)  # TODO
print('kik_post: NUM_JOINTS=', NUM_JOINTS, 'NUM_LIMBS=', NUM_LIMBS)
...
def find_peaks(param, img):
    ...
    in the image
    """
    # peaks_binary = (maximum_filter(img, footprint=generate_binary_structure(2, 2)) == img) * (img > param['thre1'])
    ...
    # a = maximum_filter(img, footprint=generate_binary_structure(2, 1))
    # A = a == img
    # B = img > param['thre1']
    # peaks_binary = A * B
    ...
    # a = binary_dilation(a, structure=struct).astype(a.dtype)
    # a = binary_dilation(a, structure=struct).astype(a.dtype)
    ...
    # a[r+1, r+1] = 1
    # struct = generate_binary_structure(2, 1)
    # a = binary_dilation(a, structure=struct).astype(a.dtype)  # r=1, kernel=3
    # a = binary_dilation(a, structure=struct).astype(a.dtype)  # 5
    # a = binary_dilation(a, structure=struct).astype(a.dtype)  # 7
    # a = binary_dilation(a, structure=struct).astype(a.dtype)  # 9

    # fp = a

    # peaks_binary = (maximum_filter(img, footprint=fp) == img) * (img > param['thre1'])
...
def find_center(img):
    T = 0.5  # 将heatmap大于阈值T的视为检测到关键点区域
    # 图像二值化
    h, w = img.shape
    binary_img = np.zeros((h, w), dtype=np.uint8)
    binary_img[img > T] = 1

    labelled_mask, num_connected_components = label(binary_img, return_num=True)
    kp = []
    for i in np.arange(num_connected_components):
        kp_i = np.mean(np.where(labelled_mask == i + 1), axis=1)[::-1]
        kp_i = kp_i.astype(np.int32)
        kp.append(kp_i)

    kp = np.array(kp)
    # if len(kp)==1:
    #     kp = np.expand_dims(np.array(kp), axis=0)
    return kp
...
def NMS(param, heatmaps, upsampFactor=1., bool_refine_center=True, bool_gaussian_filt=False, solid=False):
    """
    ...
    for joint in range(NUM_JOINTS):
        ...
        # peak_coords = find_peaks(param, map_orig)  # only do NMS in one channel
        if solid:
            peak_coords = find_center(map_orig)
        else:
            peak_coords = find_peaks(param, map_orig)  # only do NMS in one channel

        # TODO do NMS in all channels
        # print('joint', joint, 'peak_coords', peak_coords)
        ...
        if len(joints_src) == 0 or len(joints_dst) == 0:
            ...
            for i, joint_src in enumerate(joints_src):
                ...
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    # ---- 1
                    # score_dist = score_intermed_pts.mean() + min(0.1 * paf_upsamp.shape[0] / limb_dist - 1, 0)

                    # ---- 2
                    # if limb_type == 1 or limb_type == 3:  # short limbs of pallet
                    #     _w = 0.2
                    # else:
                    #     _w = 0.9    # limb_type==0 or 2   long limbs of pallet
                    # _p = 0.8
                    # score_dist = score_intermed_pts.mean() + min(_w * paf_upsamp.shape[0] / limb_dist - 1, 0)
                    ...
                    #     _w = 0.9    # limb_type==0 or 2   long limbs of pallet
                    # _p = 0.8
                    # score_dist = score_intermed_pts.mean() + 0.5 * min(_w * paf_upsamp.shape[0] / limb_dist - 1, 0)

                    # ---- 4
                    # f = 0.5
                    # penalizing short: we prefer limb_dist > _s * paf_upsamp.shape[1] ( or paf_upsamp.shape[0])
                    # penalizing long: we prefer limb_dist < _w * paf_upsamp.shape[1]
                    if limb_type == 1 or limb_type == 3:  # short limbs of pallet
                        _w = 0.6
                        _s = 0.2
                        score_penalizing_short_dist = 0.45 * min(limb_dist / ( _s * paf_upsamp.shape[0]) - 1, 0)
                        score_penalizing_long_dist = 0.55 * min(_w * paf_upsamp.shape[0] / limb_dist - 1, 0)
                    else:
                        _w = 0.6    # limb_type==0 or 2   long limbs of pallet
                        _s = 0.2
                        score_penalizing_short_dist = 0.55 * min(limb_dist / ( _s * paf_upsamp.shape[1]) - 1, 0)
                        score_penalizing_long_dist = 0.45 * min(_w * paf_upsamp.shape[1] / limb_dist - 1, 0)

                    score_dist = score_intermed_pts.mean() + score_penalizing_long_dist + score_penalizing_short_dist

                    # print('limb_type', limb_type, 'limb_dist', limb_dist, 'score_dist ', score_dist,
                    #       'score_penalizing_short_dist', score_penalizing_short_dist,
                    #       'score_penalizing_long_dist',  score_penalizing_long_dist)

                    _p = 0.8
                    # Criterion 1: At least 80% of the intermediate points have a score higher than thre2
                    criterion1 = (np.count_nonzero(score_intermed_pts > param['thre2']) > _p * num_intermed_pts)
                    # Criterion 2: Mean score, penalized for large limb
                    # distances (larger than half the image height), is positive
                    criterion2 = (score_dist > 0)
                    if criterion1 and criterion2:
                        # Last value is the combined paf(+limb_dist) + heatmap
                        # scores of both joints
                        connection_candidates.append(
                            [i, j, score_dist, score_dist + joint_src[2] + joint_dst[2]])
            ...
            connection_candidates = sorted(connection_candidates, key=lambda x: x[3], reverse=True)
    ...
    for limb_type in range(NUM_LIMBS):
        ...
        for limb_info in connected_limbs[limb_type]:
            ...
            if len(person_assoc_idx) == 1:
                ...
                # row = -1 * np.ones(20)
                row = -1 * np.ones(NUM_JOINTS+2)  # Mod by Jie
                ...
                # print('limb_type', limb_type, 'person_to_joint_assoc.append(row)', row)

    # Delete people who have very few parts connected
    # TODO 依据卡板进行修改：卡板的点较少，阈值设小一点
    T_min_number = 3
    T_min_avgscore = 0.2
    people_to_delete = []
...
def plot_pose(img_orig, joint_list, person_to_joint_assoc, bool_fast_plot=True):
    ...
    for limb_type in range(NUM_LIMBS):  # Mod by jie
...
def decode_pose(img_orig, param, heatmaps, pafs, viz=True, pp=False, solid=False):
    # Bottom-up approach:
    # Step 1: find all joints in the image (organized by joint type: [0]=nose,
    # [1]=neck...)
    joint_list_per_joint_type = NMS(param,
                                    heatmaps,
                                    img_orig.shape[0] / float(heatmaps.shape[0]),
                                    bool_refine_center=False,
                                    solid=solid)

    #
    # ---- post process here
    # 查看每个类别的点的数量，从数量较多的点中删除
    if pp:
        # print('pp in decode_pose')
        joint_list_per_joint_type = pallet_post_kp_list(joint_list_per_joint_type)
    ...
    # ---  增加过滤： 将同一类别中，过于靠近的点中，去除得分较低的点
    # 查看每个类别的点的数量，从数量较多的点中删除
    # pallet_post_kp(joint_list)
    ...
    connected_limbs = find_connected_joints(param, paf_upsamp, joint_list_per_joint_type, num_intermed_pts=10)
    # connected_limbs:
    # 每个limb 对应一个数组: [joint_src_id, joint_dst_id, limb_score_penalizing_long_dist,joint_src_index,joint_dst_index]
    # print('connected_limbs', len(connected_limbs), connected_limbs)

    # Step 3: associate limbs that belong to the same pallet
    pallet_to_joint_assoc = group_limbs_of_same_person(connected_limbs, joint_list)
    # 2d np.array of size num_people x (NUM_JOINTS+2). For each person found:
    # First NUM_JOINTS columns contain the index (in joint_list) of the joints associated
    # with that person (or -1 if their i-th joint wasn't found)
    # 2nd-to-last column: Overall score of the joints+limbs that belong to this person
    # Last column: Total count of joints found for this person
    ...
    # ---------------------------------------------------
    # ---- The best location to add post-processing  ----
    # ---------------------------------------------------
    if pp:
        # pallet_to_joint_assoc = pallet_post(joint_list, connected_limbs, pallet_to_joint_assoc, flag)
        pallet_to_joint_assoc = pallet_post_3to4th(joint_list, connected_limbs, pallet_to_joint_assoc)

    # (Step 4): plot results
    if viz:
        to_plot, canvas = plot_pose(img_orig, joint_list, pallet_to_joint_assoc)
    else:
        to_plot = None
        canvas = None

    return to_plot, canvas, joint_list, connected_limbs, pallet_to_joint_assoc
